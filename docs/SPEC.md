...
- 理由: 当時コミットされていた `dist/` と `types/` は 4.9.5 の出力だった（その後 D-3 の改訂で追跡をやめた）。構成変更と同時にコンパイラを上げると、ディレクトリ移動による差分とコンパイラ更新による差分が混ざり、どちらが原因の変化か切り分けられなくなる。ルートの 5.9.3 はビルドにも型チェックにも使われていなかった。
