...
- 決定: ビルド生成物の `dist/` と `types/` を git 管理下に置く。
...
### D-9: TypeScript は 4 系に揃える（単一化に伴う統一）

- 日付: 2026-09-20
- 決定: 単一パッケージ化で1つに定まる TypeScript のバージョンは、`packages/core` が使っていた `^4.6.2`（実解決 4.9.5）を採用する。ルートの devDependency にあった `^5.9.3` は破棄する。
- 理由: コミット済みの `dist/` と `types/` は 4.9.5 の出力（→ D-3）。構成変更と同時にコンパイラを上げると、ディレクトリ移動による差分とコンパイラ更新による差分が混ざり、どちらが原因の変化か切り分けられなくなる。ルートの 5.9.3 はビルドにも型チェックにも使われていなかった。
- 却下した案: この機に 5 系へ上げる — 上記のとおり差分の切り分けができなくなる。
- 覆す条件: 構成変更が落ち着いた後、単独の変更として更新する（→ TASK-TREE T-5）。
