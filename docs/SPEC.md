...
- **利用者への影響**: これまで `authorize` 未実装のユースケースは実行時に例外で止まっていた。この変更後は止まらずに実行される。認可のつもりで例外に頼っていたコードがあれば認可漏れになるので、`authorize` を書いていないユースケースが意図どおりかを確認すること。挙動の変更にあたるため patch では出せない。npm への初回公開を 1.2.0（minor）としたのはこのため（→ D-15）。
...
### D-15: バージョン番号は GitHub Packages 時代から継承する

- 日付: 2026-09-22
- 決定: npm への初回公開は 1.2.0。GitHub Packages で配っていた v1.1.5 の続きとして番号を継承し、振り直さない。
- 却下した案: レジストリ移行を機に 1.0.0 から始める — 利用者が実質ひとりで、移行のきっかけとしては筋が通っていたが、既存の git タグ `v1.0.0`（2025-07-22 のコミット `5f66b6d` を指す）と衝突する。タグを消して打ち直せば過去のリリース地点の目印が失われ、npm は公開から 72 時間を過ぎると取り消せないため番号の選び直しもきかない。得るものに対して代償が大きい。
- minor である理由: `authorize` の挙動変更（→ D-12）を含むため patch では出せない。

### D-16: GitHub Release はワークフローで自動作成する

- 日付: 2026-09-22
- 決定: `publish.yml` の publish 成功後に `gh release create` で Release を作る。ノートは `--generate-notes` に任せる。
- 経緯: タグを push しても GitHub Release は作られない。そのため 1 年前に手動で作られた `v1.0.0` だけが Release として残り、v1.1.x も v1.2.0 もタグのみで、リポジトリの Releases 欄は古い v1.0.0 を Latest と表示し続けていた。
- 理由: 手で作る運用は、今回のように忘れられて実態と食い違う。publish が通ったときだけ Release ができるよう、同じワークフローの後段に置く。
- 実装上の注意:
  - `permissions` を `contents: read` から `contents: write` に上げる必要がある。publish 自体は `NPM_TOKEN` で認証しているので、この権限は Release 作成のためだけのもの。
  - `--verify-tag` を付けて、タグが存在しない場合は失敗させる。
  - `gh` は `ubuntu-latest` に同梱されているので追加インストールは不要。`GH_TOKEN` に `github.token` を渡すだけでよい。
- 適用範囲: 次にタグを push するときから。`v1.2.0` は既にタグを push した後なので遡って作られず、Release は手動で作る。
- 関連: リリースノートの中身は将来 `CHANGELOG.md`（→ TASK-TREE T-9）と重複しうる。どちらを正とするかは T-9 で決める。
